import 'package:dartz/dartz.dart';
import 'package:laza_e_commerce/Features/home/presentation/view/home_tab/domain/repos/delete_fav_repo.dart';
import '../../../../../../../Core/utils/failure.dart';

class DeleteFavUseCase extends UseCase<void, WishlistParams> {
  final DeleteFavRepo deleteFavRepo;
  var token =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..zyUA93GL-uGSLcZeIkNJR52MQGRWvygqv7tvzaZ335I";

  DeleteFavUseCase(this.deleteFavRepo);
  @override
  Future<Either<Failure, void>> call(WishlistParams param) async {
    return await deleteFavRepo.deleteFav(
        token: token,
        productId: param.productId,
        image: param.image,
        name: param.name,
        price: param.price);
  }
}

abstract class UseCase<Type, Params> {
  Future<Either<Failure, Type>> call(Params params);
}

class WishlistParams {
  final String productId, name, image;
  final double price;

  WishlistParams({
    required this.productId,
    required this.name,
    required this.image,
    required this.price,
  });
}
